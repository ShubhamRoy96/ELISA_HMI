...
void Calculation(int i)
{
  ...
  //{
    ...
    //float VoltageRead = VoltageRead + VoltageOut / 1024.0 * 5.0;
  //}
  //VoltageAverage = (VoltageRead / 5);
  if(i==0)VoltageAverage = 10.0;
  else if(i==1) VoltageAverage = 20.0;
  else if(i==2) VoltageAverage = 30.0;
  else if(i==3) VoltageAverage = 40.0;
  else if(i==4) VoltageAverage = 50.0;
  else if(i==5) VoltageAverage = 60.0;
  else VoltageAverage = 100.0;
  //Data[i] = {j};
  // Motor move function goes here;
  //if(i<10)
  //i++;
  //else
  //i=0; 
  ...
  //TLCD.setColor(VGA_BLACK);
  //TLCD.setFont(BigFont);
  //TLCD.printNumF(Data[j], 1, 255 ,204);
 // delay(3000);
  //TLCD.print("  ", 255 ,204);
  ...
  EDB_Status result = db.appendRec(EDB_REC dataEvent);
  if(i<10) i++;
  else i=0;
  TLCD.setBackColor(VGA_WHITE); //Data on Screen
  TLCD.setColor(VGA_BLACK);
  TLCD.setFont(BigFont);
  TLCD.printNumF(dataEvent.data, 1, 255 ,204);
